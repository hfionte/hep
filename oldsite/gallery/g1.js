...
function setPreview(el) {
   ...
   previewImg.setAttribute(CL,"selected");
}
...
function openGalleryListThis() {
   setPreviewFirst( activateImageSet(this.parentNode.getElementsByTagName("dd")[0]) );
   openGalleryList(this);
}
...
function activateImageSetThis() {
   setPreviewFirst( activateImageSet(this) );
}
...
function setFirstSet(galleryEl) {
   ...
   ddList[0].setAttribute(CL,"open");
   for (var i=1; i<length.ddList; i++) {
      ddList[i].setAttribute(CL,"closed");
   }
